    ...
    public function generar_reporte(Request $request)
    {
        if ($request->clave_prod != '') {
            $request->validate([
                ...
                'desde.after' => 'Fecha inaccesible! Mínimo desde el año 2020',
                'hasta.before' => 'Fecha inaccesible! Máximo hasta el dia de hoy',
            ]);
            $producto = Producto::where('clave_prod',  $request->clave_prod)
                ->first();
        } else {
            $request->validate([
                'tipo_prod' => 'required',
